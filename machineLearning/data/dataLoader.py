...
def getSplitIndices(dataAmount: int, split: int) -> list[int]:
    """
...
class DataSet(object):
    ...
    def __init__(self, data: np.ndarray, *, adjacency: np.ndarray = None, labels: np.ndarray = None, targets: np.ndarray = None, weights: np.ndarray = None, classWeights: np.ndarray = None) -> None:
        self.data = data
        ...
        self.labels = labels
        self.uniques = np.unique(self.labels, axis=0) if not labels is None else None # number of different dataset in this data set
    ...
    def __getitem__(self, index: int | np.ndarray) -> 'DataSet':
        """
    ...
    def selectFeatures(self, featureIndices: ArrayLike) -> 'DataSet':
        """
    ...
    def splitDataSet(self, splitIndices: ArrayLike) -> 'DataSet':
        """
...
class DataLoader():
    ...
    def __init__(self, dataSet: DataSet, batchSize: int, *, shuffle: bool = False, kFold: int = 1, usBootstrapping: bool = False, numTrainingSamples: int = 1) -> None:
        ...
        if kFold > 1 and (numTrainingSamples > 1 and usBootstrapping == True):
        ...
        if (numTrainingSamples > 1) and (usBootstrapping == True):
    ...
    def trainingSamples(self, numTrainingSamples: int = None):
        """
