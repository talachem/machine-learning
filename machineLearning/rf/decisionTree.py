...
class DecisionTree():
    ...
    # initializer
    def __init__(self, maxDepth: int = None, minSamplesSplit: int = 2) -> None:
    ...
    def train(self, data: np.ndarray | DataSet, targets: np.ndarray = None, classWeights: np.ndarray = None) -> None:
        """
        training the tree
        """
    ...
    def eval(self, data: np.ndarray | DataSet) -> np.ndarray:
        """
    ...
    def raw(self, data: np.ndarray | DataSet) -> list:
        """
        make predictions from the trained tree
        """
        if self._trained is False:
    ...
    def _traverse(self, node: Node, dataPoint: np.ndarray) -> Node:
        """
    ...
    def __len__(self) -> int:
        ...
        number of leafs
        """
        return len(self.self.breadthFirst())
